...
function removeProperties(obj, keys) {
  ...
  keys.forEach((key) => {
    if (key in cloneObj) delete cloneObj[key];
  });
...
function compareObjects(obj1, obj2) {
  const keys1 = Object.values(obj1).join('');
  const keys2 = Object.values(obj2).join('');
...
function makeWord(lettersObject) {
  ...
  Object.keys(lettersObject).forEach((key) => {
    Object.values(lettersObject[key]).forEach((value) => {
      stringArr[value] = key;
      return value;
    });
  });
...
function sellTickets(queue) {
  ...
  const canSell = queue.every((pay) => {
    sellerMoney += ticketCost;
    if (pay > ticketCost) {
      if (sellerMoney < pay - ticketCost) {
        return false;
      }
      sellerMoney += ticketCost;
      sellerMoney -= pay - ticketCost;
    }
    return true;
  });
  return canSell;
